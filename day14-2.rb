...
(1..improve_turn+10).each do |turn|
    ...
    # puts recipies.map(&:to_s).join("")

    # if recipies.size > 6
    #     puts "last 6 recipies scores #{recipies[-6..-1].map(&:to_s).join("").to_i}"
    # end

    if recipies.size > 6 && recipies[-6..-1].map(&:to_s).join("").to_i == 793031
